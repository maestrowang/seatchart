...
(async()=>{
  ...
  console.log('=== Row 4/5 (straight) positions BEFORE ===');
  const before = window.eval(`
    JSON.stringify({
      row4: chart.seats.filter(s=>s.row===4).map(s=>({x:Math.round(s.x*100)/100,y:Math.round(s.y*100)/100})),
      row5: chart.seats.filter(s=>s.row===5).map(s=>({x:Math.round(s.x*100)/100,y:Math.round(s.y*100)/100}))
    })
  `);
  console.log(before);
  ...
  console.log('=== Row 4/5 positions AFTER a SMALL nudge (105%) ===');
