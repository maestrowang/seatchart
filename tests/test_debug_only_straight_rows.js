...
(async()=>{
  ...
  // Select ONLY seats in the two straight rows (4 and 5), matching the literal bug report
  await reload();
  const straightIds = window.eval("chart.seats.filter(s=>s.row===4||s.row===5).map(s=>s.id)");
  console.log('Selected seat count (only rows 4 & 5):', straightIds.length);
  ...
  console.log('=== Test A: SEAT SPACING (only straight rows selected) ===');
  ...
  await reload();
  console.log('=== Test B: ROW SPACING (only straight rows selected) ===');
  window.eval(`setSelection(${JSON.stringify(straightIds)})`);
  await wait(10);
  ...
  await reload();
  console.log('=== Test C: STAGE SPACING (only straight rows selected) ===');
  window.eval(`setSelection(${JSON.stringify(straightIds)})`);
  await wait(10);
